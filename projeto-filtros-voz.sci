...
elseif filterType == "notch" then
    ...
    w = linspace(0, 4*%pi, 1024);
    
    // Simplificação do cálculo de hejw
    num_poly = poly(flipdim(num, 2), 'z', 'c');
    den_poly = poly(flipdim(den, 2), 'z', 'c');
    hejw = horner(num_poly, %e^(-%i*w)) ./ horner(den_poly, %e^(-%i*w));
    
    phase = atan(imag(hejw), real(hejw))
    subplot(321);
    plot2d(w, phase);
    title(gettext("Fase do filtro"));
    xlabel("w");
    ylabel("angulo em rad");
end
